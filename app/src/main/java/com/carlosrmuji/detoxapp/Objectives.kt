...
fun Objectives(
    ...
    LaunchedEffect(Unit) {
        ...
        // 2) Validación de fecha de fin de fase
        try {
        ...
        messages.find { it.getString("timestamp")?.startsWith(today) == true }
            ?.let { idx ->
                getPhaseChallenges(phase.value).indexOfFirst { it.title == "Reflexión diaria" }
            }?.takeIf { it >= 0 }
